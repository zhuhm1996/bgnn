...
def preprocess_bilinear(adj):
    ...
    D_hop1_all = adj_hop1_all.sum(1)
    N_hop1_all = 0.5 * np.multiply(D_hop1_all, D_hop1_all-np.ones((adj.shape[0])))
    N_hop1_all = np.diag(1. / N_hop1_all)
    N_hop1_all[np.isinf(N_hop1_all)] = 0.

    D_hop2_all = adj_hop2_all.sum(1)
    N_hop2_all = 0.5 * np.multiply(D_hop2_all, D_hop2_all - np.ones(adj.shape[0]))
    N_hop2_all = np.diag(1. / N_hop2_all)
    N_hop2_all[np.isinf(N_hop2_all)] = 0.

    N_hop1_all = sp.coo_matrix(N_hop1_all)
    N_hop2_all = sp.coo_matrix(N_hop2_all)
    ...
    return sparse_to_tuple(adj_hop1_all), sparse_to_tuple(adj_hop2_all),\
           ...
           sparse_to_tuple(N_hop1_all), sparse_to_tuple(N_hop2_all)
