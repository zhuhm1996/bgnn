...
def load_data(dataset_str): # {'pubmed', 'citeseer', 'cora'}
    ...
    objects = []
    datafile = '/gdata1/zhuhm/'
    for i in range(len(names)):
        with open(datafile+"data/ind.{}.{}".format(dataset_str, names[i]), 'rb') as f:
    ...
    test_idx_reorder = parse_index_file(datafile+"data/ind.{}.test.index".format(dataset_str))
...
def preprocess_bilinear(adj):
    ...
    D_hop1_neig = adj_hop1_neig.sum(1)
    N_hop1_neig = 0.5 * np.multiply(D_hop1_neig, D_hop1_neig-np.ones((adj.shape[0])))
    N_hop1_neig = np.diag(1. / N_hop1_neig)
    N_hop1_neig[np.isinf(N_hop1_neig)] = 0.

    D_hop2_neig = adj_hop2_neig.sum(1)
    N_hop2_neig = 0.5 * np.multiply(D_hop2_neig, D_hop2_neig - np.ones(adj.shape[0]))
    N_hop2_neig = np.diag(1. / N_hop2_neig)
    N_hop2_neig[np.isinf(N_hop2_neig)] = 0.

    N_hop1_neig = sp.coo_matrix(N_hop1_neig)
    N_hop2_neig = sp.coo_matrix(N_hop2_neig)
    ...
    return sparse_to_tuple(adj_hop1_all), sparse_to_tuple(adj_hop2_all),\
                                  sparse_to_tuple(adj_hop1_neig), sparse_to_tuple(adj_hop2_neig),\
           sparse_to_tuple(N_hop1_neig), sparse_to_tuple(N_hop2_neig)
