...
flags.DEFINE_float('feadrop', 0.6, 'feature Dropout rate (1 - keep probability).')
...
flags.DEFINE_float('alpha', 0.5, 'alpha balancing BGCN')
flags.DEFINE_float('beta', 0.5, 'beta balancing Bilinear aggregated information')
residual = False
...
#biases = process.preprocess_adj_gat(adj)####process adj to find the 1hop and 2hop neighbors
biases = process.preprocess_adj_bias(adj)####just 1-hop adj matrix
adj_hop1_all, adj_hop2_all, adj_hop1_neig, adj_hop2_neig, N_hop1_target, N_hop2_target = process.preprocess_bilinear(adj)

t_rep_s = time.time()

ftr_in = tf.placeholder(dtype=tf.float32, shape=(1, nb_nodes, ft_size))
bias_in = tf.sparse_placeholder(dtype=tf.float32)
adj_hop1_all_in = tf.sparse_placeholder(dtype=tf.float32)
adj_hop2_all_in = tf.sparse_placeholder(dtype=tf.float32)
adj_hop1_neig_in = tf.sparse_placeholder(dtype=tf.float32)
adj_hop2_neig_in = tf.sparse_placeholder(dtype=tf.float32)
N_hop1_target_in = tf.sparse_placeholder(dtype=tf.float32)
N_hop2_target_in = tf.sparse_placeholder(dtype=tf.float32)
...
logits = model.inference(ftr_in, nb_classes, nb_nodes, is_train,
                            attn_drop, ffd_drop,
                            bias_mat=bias_in,
                            adj_hop1_all_mat=adj_hop1_all_in,
                            adj_hop2_all_mat=adj_hop2_all_in,
                            adj_hop1_neig_mat=adj_hop1_neig_in,
                            adj_hop2_neig_mat=adj_hop2_neig_in,
                            N_hop1_target_mat=N_hop1_target_in,
                            N_hop2_target_mat=N_hop2_target_in,
                            hid_units=[FLAGS.hidden1], n_heads=[8,FLAGS.head],
...
for epoch in range(FLAGS.epochs):
    ...
    _, loss_value_tr, acc_tr = sess.run([train_op, loss, accuracy],
        feed_dict={
            ftr_in: features[0:1],
            bias_in: bbias,
            adj_hop1_all_in: adj_hop1_all,
            adj_hop2_all_in: adj_hop2_all,
            adj_hop1_neig_in: adj_hop1_neig,
            adj_hop2_neig_in: adj_hop2_neig,
            N_hop1_target_in: N_hop1_target,
            N_hop2_target_in: N_hop2_target,
            lbl_in: y_train[0:1],
